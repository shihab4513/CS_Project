...
### 4. Run a free local LLM review in Google Colab

`run_local_llm_review.py` uses `Qwen/Qwen2.5-1.5B-Instruct` locally inside the
Colab runtime. It does not require an API key. The reviewer receives only a
bounded summary of the already-created JSON evidence report: package metadata,
configured source findings, dynamic summary, and triage action. It does not
receive raw package source code, execute a package, or replace human review.

Use a Colab GPU when available, install the optional requirements, then run:

```python
!pip -q install -r requirements-colab-llm.txt
!python run_local_llm_review.py \
  --evidence /content/new_package_evidence_report.json \
  --output /content/llm_review.json
```

The result preserves the raw model response and best-effort structured JSON.
Use it as an evidence-grounded reviewer explanation only. It is not a
ground-truth label, a calibrated probability, or proof that a package is safe.
...
For the optional local LLM stage, use
`notebooks/local_llm_evidence_review_colab.ipynb` after creating a source/trace
evidence report on Windows.
